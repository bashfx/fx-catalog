...
# DOC_USAGE_START
#
#   ${bld}${SELF_NAME} v3.0.0${x} - A smart domain availability watcher.
#
#   ${orange}${utri}${x} ${bld}USAGE:${x}
#   ${blue}watchdom ${grey}<command> [args...]${x}
#   ${grey}Legacy compatibility: 'watchdom <domain> [options]' is aliased to 'watchdom watch ...'${x}
#
#   ${orange}${utri}${x} ${bld}CORE COMMANDS:${x}
#   ${green}watch ${grey}<domain>${x}          ${grey}Monitor a domain. Runs a one-time query by default.${x}
#   ${green}time ${grey}<datetime>${x}         ${grey}Standalone time countdown utility.${x}
#
#   ${orange}${utri}${x} ${bld}TLD MANAGEMENT:${x}
#   ${green}list_tlds${x}              ${grey}Show supported TLD patterns.${x}
#   ${green}add_tld ${grey}<tld> <srv> <pat>${x}  ${grey}Add custom TLD support to ~/.watchdomrc.${x}
#   ${green}test_tld ${grey}<tld> <domain>${x}   ${grey}Test TLD pattern accuracy.${x}
#
#   ${orange}${utri}${x} ${bld}ADMINISTRATION:${x}
#   ${green}install${x}                ${grey}Install the watchdom binary to the FX bin directory.${x}
#   ${red}uninstall${x}              ${grey}Remove the watchdom installation.${x}
#   ${green}status${x}                 ${grey}Show installation status and paths.${x}
#   ${green}version, --version${x}     ${grey}Display the script version.${x}
#   ${green}info, --info${x}           ${grey}Display detailed script information.${x}
#   ${green}help, -h, --help${x}       ${grey}Show this help message.${x}
#
#   ${orange}${utri}${x} ${bld}FLAGS & OPTIONS:${x}
#   ${green}-i, --interval ${grey}<sec>${x}    ${grey}Base poll interval in seconds. Default: 60.${x}
#   ${green}-n, --max-checks ${grey}<num>${x}   ${grey}Stop after N checks. Default: 0 (unlimited).${x}
#   ${green}--until ${grey}<datetime>${x}       ${grey}Target datetime (e.g., "2025-12-25 18:00:00 UTC").${x}
#   ${green}-e, --expect ${grey}<regex>${x}     ${grey}Override the 'available' pattern for this run.${x}
#   ${green}--time_local${x}           ${grey}Display countdown timestamps in local time only.${x}
#   ${green}-f, --force${x}             ${grey}Force overwrite when using 'add_tld'.${x}
#   ${green}-y, --yes${x}               ${grey}Answer 'yes' to all confirmation prompts.${x}
#   ${green}-d, --debug${x}             ${grey}Enable informational, success, and warning messages.${x}
#   ${green}-t, --trace${x}             ${grey}Enable verbose trace messages (implies -d).${x}
#   ${green}-q, --quiet${x}             ${grey}Suppress all output except errors.${x}
#
#   ${orange}${utri}${x} ${bld}POLLING PHASES:${x}
#   ${blue}${lambda} [PRE]${x}    ${grey}>30min before target: base interval.${x}
#   ${red}${delta} [HEAT]${x}   ${grey}≤30min before target: ramp to 10s.${x}
#   ${purple}${delta} [GRACE]${x}  ${grey}0-3hrs after target: stick at 10s.${x}
#   ${blue}${spark} [COOL]${x}   ${grey}>3hrs after target: progressive cooldown.${x}
#
#   ${orange}${utri}${x} ${bld}EMAIL NOTIFICATIONS:${x}
#   ${grey}Set all of the following environment variables to enable email alerts:${x}
#   ${grey}NOTIFY_EMAIL, NOTIFY_FROM, NOTIFY_SMTP_HOST, NOTIFY_SMTP_PORT, NOTIFY_SMTP_USER, NOTIFY_SMTP_PASS${x}
#
#   ${orange}${utri}${x} ${bld}EXIT CODES:${x}
#   ${grey}  0: Success (pattern matched)             2: Bad arguments${x}
#   ${grey}  1: Not found (max checks/exit)           3: Missing dependencies${x}
#
# DOC_USAGE_END
